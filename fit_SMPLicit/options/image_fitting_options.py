...
class FitOptions:
    def __init__(self):
        self._parser = argparse.ArgumentParser(description="Image fitting options for SMPLicit")
    ...
    def initialize(self):
        ...
        # 聚类和风格参数
        self._parser.add_argument('--z_dim', type=int, default=18, help='服装潜变量维度')
        ...
        self._parser.add_argument('--num_params_style', type=int, default=0, help='风格参数维度')
        self._parser.add_argument('--num_params_shape', type=int, default=0, help='形状参数维度')
        ...
        self._parser.add_argument('--output_folder', type=str, default='./outputs', help='结果保存目录')
        self._parser.add_argument('--image_folder', type=str, default='data/images/', help='输入图像文件夹')
        self._parser.add_argument('--smpl_prediction_folder', type=str, default='data/smpl_prediction/',
                                  help='SMPL 预测结果文件夹')
        self._parser.add_argument('--cloth_segmentation_folder', type=str, default='data/cloth_segmentation/',
                                  help='服装分割图像文件夹')
        self._parser.add_argument('--instance_segmentation_folder', type=str, default='data/instance_segmentation/',
                                  help='实例分割图像文件夹')
        self._parser.add_argument('--image_extension', type=str, default='.jpg', help='图像扩展名（.png, .jpg 等）')
        self._parser.add_argument('--z_dim', type=int, default=18, help='服装潜变量维度（默认 18）')
        self._parser.add_argument('--lr', type=float, default=0.01, help='学习率')
        self._parser.add_argument('--lr_decayed', type=float, default=0.0003, help='衰减后的学习率')
        self._parser.add_argument('--step', type=int, default=10000, help='步长')
        self._parser.add_argument('--iterations', type=int, default=200, help='优化迭代次数')
        self._parser.add_argument('--index_samples', type=int, default=100, help='采样点数')
        ...
        self._parser.add_argument('--num_clusters', type=int, default=500, help='聚类数量')
        self._parser.add_argument('--clamp_value', type=float, default=0.5, help='截断阈值')
        self._parser.add_argument('--num_params_style', type=int, default=12,
                                  help='风格参数维度 (默认 upperbody_n_z_style)')
        self._parser.add_argument('--num_params_shape', type=int, default=6,
                                  help='形状参数维度 (默认 upperbody_n_z_cut)')
        self._parser.add_argument('--other_labels', type=str, default='', help='额外的 parsing label 列表')
        self._parser.add_argument('--repose', action='store_true', help='是否重新姿态化')
        self._parser.add_argument('--b_min', type=str, default='', help='边界最小值')
        self._parser.add_argument('--b_max', type=str, default='', help='边界最大值')
        ...
        self._parser.add_argument('--path_SMPL', type=str,
                                  default=path_SMPLicit + 'SMPLicit/utils/neutral_smpl_with_cocoplus_reg.txt')
    ...
    def parse(self):
        ...
        os.makedirs(self._opt.output_folder, exist_ok=True)
        return self._opt
