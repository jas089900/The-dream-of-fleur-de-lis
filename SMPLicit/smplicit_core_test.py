...
class Model():
    def __init__(self, filename, n_z_cut, n_z_style, num_clusters, name_clusters, b_min, b_max, resolution, thresh=-0.05):
        ...
        self.clusters = np.load('../clusters/indexs_clusters_tshirt_smpl.npy', allow_pickle=True)
        self.b_min = b_min
        self.b_max = b_max
    ...
    def load(self):
        # load G
        self._G.load_state_dict(torch.load(self.filename, map_location=torch.device('cpu')))
