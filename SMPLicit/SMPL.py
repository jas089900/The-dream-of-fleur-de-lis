...
class SMPL(nn.Module):
    def __init__(self, model_path, joint_type = 'cocoplus', obj_saveable = False):
        ...
        np_J_regressor = np.array(model['J_regressor'], dtype = np.float64)
        ...
        np_joint_regressor = np.array(model['cocoplus_regressor'], dtype = np.float64)
        if joint_type == 'lsp':
        ...
        np_weights = np.array(model['weights'], dtype = np.float64)
    ...
    def forward(self, beta, theta, get_skin = False, theta_in_rodrigues=True):
        ...
        v_posed_homo = torch.cat([v_posed, torch.ones(num_batch, v_posed.shape[1], 1)], dim = 2)
    ...
    def deform_clothes_smpl_usingseveralpoints(self, theta, J, v_smpl, v_cloth, neighbors = 3):
        ...
        v_posed_homo_smpl = torch.cat([v_posed_smpl, torch.ones(num_batch, v_posed_smpl.shape[1], 1)], dim = 2)
        v_posed_homo_cloth = torch.cat([v_posed_cloth, torch.ones(num_batch, v_posed_cloth.shape[1], 1)], dim = 2)
    ...
    def deform_clothed_smpl_w_normals(self, theta, J, v_smpl, v_cloth, v_normals):
        ...
        v_normals_posed = torch.cat([v_normals, torch.ones(num_batch, v_normals.shape[1], 1, )], dim=2)
    ...
    def unpose_and_deform_cloth(self, v_cloth_posed, theta_from, theta_to, beta, Jsmpl, vsmpl, theta_in_rodrigues=True):
        ...
        v = torch.cat([v_cloth_posed, torch.ones(len(v_cloth_posed), 1,)], 1)
    ...
    def unpose_and_deform_cloth_w_normals(self, v_cloth_posed, v_normals, theta_from, theta_to, beta, Jsmpl, vsmpl):
        ...
        v_normals_posed = torch.cat([v_normals.unsqueeze(0), torch.ones(num_batch, v_normals.shape[0], 1)], dim=2)
...
if __name__ == '__main__':
    ...
    smpl = SMPL(os.path.join(os.path.dirname(__file__),'model/neutral_smpl_with_cocoplus_reg.txt'), obj_saveable = True).cpu()
    pose= np.array([
    ...
    vbeta = torch.tensor(np.array([beta])).float().cpu()
    vpose = torch.tensor(np.array([pose])).float().cpu()
    ...
    vpose = torch.tensor(np.array([rpose])).float().cpu()
